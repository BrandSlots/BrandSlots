      ...
      pw.MultiPage(
        ...
        build: (ctx) {
          ...
          for (int i = 0; i < mediaSummaries.length; i++) {
            ...
            // Per-screen line: rounds in date range + last activity
            final screenLines = <String>[];
            for (final row in assignedScreenRows) {
              final last = row.screen.lastPlaybackAt;
              final screenRoundsInRange = row.byDate.values.fold(0, (a, b) => a + b);
              screenLines.add(
                '${row.screen.name}: $screenRoundsInRange rounds  |  last: ${_fmtTs(last)}',
              );
            }
            ...
            final totalMediaRoundsInRange = periodRounds.values.fold(0, (a, b) => a + b);
            final playTimeInRange = totalMediaRoundsInRange * item.durationSeconds;
