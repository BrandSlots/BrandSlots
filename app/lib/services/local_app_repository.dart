...
class LocalAppRepository implements AppRepository {
  ...
  Future<void> deleteClient(String clientId) async {
    ...
    _data = _data.copyWith(
      ...
      mediaItems:
          _data.mediaItems.where((item) => item.clientId != clientId).toList(),
  ...
  Future<void> recordMediaAssigned({required String screenId, required String mediaId, required int atRound}) async {
    ...
    final updated = _data.assignments.map((a) {
      if (a.screenId == screenId && a.mediaId == mediaId && a.removedAtRound == null) {
        return a.copyWith(removedAtRound: atRound);
      }
      return a;
    }).toList();
    updated.add(ScreenMediaAssignment(
      id: _id(),
      screenId: screenId,
      mediaId: mediaId,
      assignedAtRound: atRound,
      removedAtRound: null,
    ));
    _data = _data.copyWith(assignments: updated);
    await _persist();
  }
  ...
  Future<void> recordMediaRemoved({required String screenId, required String mediaId, required int atRound}) async {
    _data = _data.copyWith(
      assignments: _data.assignments.map((a) {
        if (a.screenId == screenId && a.mediaId == mediaId && a.removedAtRound == null) {
          return a.copyWith(removedAtRound: atRound);
        }
        return a;
  ...
  Future<void> deleteMedia(String mediaId) async {
    _data = _data.copyWith(
      ...
      playbackStats:
          _data.playbackStats.where((item) => item.mediaId != mediaId).toList(),
      screens: _data.screens
          .map(
            (screen) => screen.copyWith(
              assignedMediaIds: screen.assignedMediaIds
  ...
  Future<void> updateMediaDuration(
    ...
    _data = _data.copyWith(
      mediaItems: _data.mediaItems
          .map((item) => item.id == mediaId
              ? item.copyWith(durationSeconds: durationSeconds)
              : item)
          .toList(),
    );
    await _persist();
