...
class _AdminDashboardPageState extends State<AdminDashboardPage> {
  ...
  _SectionMeta _metaFor(_AdminSection section) {
    switch (section) {
      ...
      case _AdminSection.reports:
        return const _SectionMeta(
          'Reports',
          'Total rounds and media breakdown per client and per screen.',
        );
    }
  }
          ...
          if (controller.clients.isEmpty)
            ...
            for (int idx = 0; idx < controller.clients.length; idx++) ...[
              _MetricListTile(
                ...
                trailing:
                    '${_sumClientPlays(controller, controller.clients[idx].id)} rounds',
              ),
                    ...
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            '${controller.playsForScreen(screen.id)} rounds',
                            style: const TextStyle(
  ...
  // Weekly: week selector (pick any day ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ snap to Monday)
  // Daily: date picker
  // Custom: start + end
                    ...
                    Expanded(
                      child: _PanelHeading(
                        ...
                        subtitle: _reportFilterClientId == null
                            ? 'Active screens, total rounds, and video breakdown.'
                            : 'Total rounds and usage per client.',
                      ),
                    ),
                  ],
                ),
                      ...
                      builder: (context) {
                        ...
                        return Column(
                          ...
                          children: [
                            TextField(
                              onChanged: (value) =>
                                  setState(() => _reportClientSearch = value),
                              decoration: InputDecoration(
                                hintText: 'Search clients in reports...',
                                prefixIcon:
                            ...
                            if (filteredClients.isEmpty)
                              const _EmptyStateCard(
                                ...
                                subtitle:
                                    'Try a different search term to find a client report.',
                                compact: true,
                              )
                            else
                              Column(
                                children: [
                                  for (int index = 0;
                                    ...
                                    if (index < filteredClients.length - 1)
                                      const SizedBox(height: 16),
                                  ],
                                ],
                              ),
  ...
  Future<void> _showEditScreenDialog(ScreenDevice screen) async {
    ...
    var selectedOrientation = screen.screenPlayerOrientation.isEmpty
        ? 'landscape'
        : screen.screenPlayerOrientation;
  ...
  Future<_ClientDraft?> _showClientForm({ClientProfile? client}) async {
    ...
    return showDialog<_ClientDraft>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: SizedBox(
          width: _dialogContentWidth(context, 460),
          child: SingleChildScrollView(
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
    ...
    await showDialog<void>(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
          ...
          content: SizedBox(
            width: _dialogContentWidth(context, 400),
  ...
  Future<void> _addMediaToScreen(ScreenDevice screen, String mediaId) async {
    ...
    if (screen.assignedMediaIds.contains(mediaId)) return;
    await widget.controller.assignMediaToScreen(screen, mediaId);
  }

  Future<void> _removeMediaFromScreen(ScreenDevice screen, String mediaId) async {
    await widget.controller.removeMediaFromScreen(screen, mediaId);
  }
            ...
            if (_period == _TimePeriod.daily)
              ...
              OutlinedButton.icon(
                ...
                label: () {
                  ...
                  final sun = mon.add(const Duration(days: 6));
                  return Text('${_formatDate(mon)} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ ${_formatDate(sun)}');
                }(),
              )
...
class _ScreenReportCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Build per-media play counts using assignment windows (same logic as totalPlaysForMedia)
    final statsByMediaId = <String, ({int plays, String? lastPlayedAt})>{};
    final currentRounds = screen.playCount;
    for (final mediaId in screen.assignedMediaIds) {
      final assignments = controller.data.assignments
          .where((a) => a.screenId == screen.id && a.mediaId == mediaId)
          .toList();
      int plays = 0;
      if (assignments.isEmpty) {
        ...
        for (final a in assignments) {
          final end = a.removedAtRound ?? currentRounds;
          plays += (end - a.assignedAtRound).clamp(0, 999999);
        }
      }
      // last played from stats
      String? lastPlayed;
      for (final stat in screenMediaStats) {
        if (stat.mediaId == '__round__') {
          lastPlayed = latestTimestamp(lastPlayed, stat.lastPlayedAt);
        }
      }
      statsByMediaId[mediaId] = (plays: plays, lastPlayedAt: lastPlayed);
    }
    return Container(
      decoration: BoxDecoration(
        color: Colors.white,
      ...
      child: Column(
        ...
        children: [
          ...
          // Videos breakdown ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â always show assigned playlist, overlay play counts
          () {
                        ...
                        child: Row(
                          children: [
                            ...
                            Container(
                              ...
                              child: Text(
                                '${screenBreakdown[i].plays} rounds  Ãƒâ€šÃ‚Â·  ${() {
    ...
    switch (_period) {
      ...
      case _TimePeriod.custom:
        if (_customFrom != null && _customTo != null) {
          return '${_customFrom!.month}/${_customFrom!.day} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å“ ${_customTo!.month}/${_customTo!.day}';
        }
        return 'Custom';
    }
  }
                  ...
                  child: Row(
                    children: [
                      ...
                      Container(
                        ...
                        child: Text(
                          '${playsByDate[sortedDates[i]]} rounds',
                          style: const TextStyle(
                            fontSize: 12,
                            fontWeight: FontWeight.w700,
                            color: Color(0xFF3B82F6),
    ...
    return Container(
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          final trailingChild = trailingWidget ??
              (trailing != null
                  ? Text(
                      trailing!,
                      style: const TextStyle(
            ...
            return Column(
              ...
              children: [
                ...
                if (trailingWidget != null || trailing != null) ...[
                  const SizedBox(height: 8),
                  Align(alignment: Alignment.centerLeft, child: trailingChild),
                ],
              ],
            );
          }
          ...
          return Row(
            children: [
              ...
              if (trailingWidget == null && trailing != null) trailingChild,
            ],
          );
        },
      ),
...
double _dialogContentWidth(BuildContext context, double desiredWidth) {
  final availableWidth = MediaQuery.sizeOf(context).width - 32;
  return availableWidth < desiredWidth ? availableWidth : desiredWidth;
}
